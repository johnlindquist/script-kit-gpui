...
# Package
**Core launcher surfaces audit** (`02-core-surfaces`)

Objective: Audit launcher, Actions, prompts, built-ins, navigation, Brain-adjacent surfaces, and app implementation changes for behavioral regressions, shortcut drift, focus/routing defects, and actual consistency improvements.

This package owns analysis of 128 changed paths. Exact complete ownership is attached as `changed-paths.txt`; `diff-stat.txt` summarizes its delta. Representative paths:
- `src/about/render.rs`
- `src/actions/builders.rs`
- `src/actions/builders/chat.rs`
- `src/actions/builders/notes.rs`
- `src/actions/builders/script_context.rs`
- `src/actions/builders/script_context/tests.rs`
...
- `src/actions/dialog_random_action_window_tests.rs`
- `src/actions/dialog_random_tests.rs`
- `src/actions/dialog_tests.rs`
- `src/actions/dialog_validation_tests.rs`
- `src/actions/dialog_window_tests.rs`
- `src/actions/mod.rs`
- `src/actions/prelude.rs`
- `src/actions/tests/dialog_builtin_validation/builtin_dialog_examples.rs`
- `src/actions/tests/dialog_builtin_validation/builtin_dialog_field_rules.rs`
- `src/actions/tests/dialog_builtin_validation/builtin_dialog_regressions.rs`
- `src/actions/tests/dialog_builtin_validation/builtin_dialog_schema_validation.rs`
- `src/actions/tests/dialog_runtime_path_tests.rs`
- `src/actions/tests/window_lifecycle.rs`
- `src/actions/types/action_model.rs`
- `src/actions/types/tests.rs`
- `src/actions/window.rs`
- `src/app_actions/handle_action/dictation_history.rs`
...
- `src/app_impl/agent_handoff/agent_chat_context_staging.rs`
...
- `src/app_impl/agent_handoff/types.rs`
- `src/app_impl/attachment_portal.rs`
- `src/app_impl/chat_actions.rs`
- `src/app_impl/filter_input_change.rs`
- … plus 88 paths listed in attached `changed-paths.txt`

The local execution worker may write only inside `.notes/oracle/branch-trust-audit/lanes/02-core-surfaces/` and, if explicitly assigned by the manager, package-specific screenshots under `.artifacts/branch-trust-audit/02-core-surfaces/`. Production files are read-only. Shared article source, integrated ledger, manifest, root config, generated outputs, and publication are manager/lane-6 owned.
