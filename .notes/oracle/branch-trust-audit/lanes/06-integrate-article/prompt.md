...
# Package
**Integration, screenshots, and article plan** (`06-integrate-article`)

Objective: Plan the integrated path accounting, representative runtime screenshot set, confidence model, article narrative, and especially a large prioritized next-steps program. Reconcile evidence without trusting lane self-reports.

This package owns analysis of 8 changed paths. Exact complete ownership is attached as `changed-paths.txt`; `diff-stat.txt` summarizes its delta. Representative paths:
- `CLAUDE.md`
- `GLOSSARY.md`
...
- `src/test_support/agent_chat_portal.rs`
- `src/warning_banner.rs`

The local execution worker may write only inside `.notes/oracle/branch-trust-audit/lanes/06-integrate-article/` and, if explicitly assigned by the manager, package-specific screenshots under `.artifacts/branch-trust-audit/06-integrate-article/`. Production files are read-only. Shared article source, integrated ledger, manifest, root config, generated outputs, and publication are manager/lane-6 owned.
