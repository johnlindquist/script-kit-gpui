...
# Package
**Windows, Flow, Notes, Dictation audit** (`04-window-flows`)

Objective: Audit Flow, Notes, Dictation, windows, lifecycle, automation surface collection, focus/visibility, and native platform changes while preserving the locked glass calibration.

This package owns analysis of 45 changed paths. Exact complete ownership is attached as `changed-paths.txt`; `diff-stat.txt` summarizes its delta. Representative paths:
- `src/dictation/delivery.rs`
...
- `src/flows/automation.rs`
- `src/flows/catalog.rs`
- `src/flows/explain_cache.rs`
- `src/flows/session.rs`
- `src/main_window_preflight/build.rs`
- `src/notes/actions_panel.rs`
- `src/notes/ai_scope.rs`
- `src/notes/day_switcher.rs`
- `src/notes/mod.rs`
- `src/notes/search_model.rs`
- `src/notes/storage.rs`
- `src/notes/window.rs`
- `src/notes/window/ai_handoff.rs`
- `src/notes/window/clipboard_ops.rs`
- `src/notes/window/contract.rs`
- `src/notes/window/focus.rs`
- `src/notes/window/init.rs`
- `src/notes/window/keyboard.rs`
- `src/notes/window/layout.rs`
...
- `src/window_resize/arg_layout.rs`
- `src/window_resize/mod.rs`
- … plus 5 paths listed in attached `changed-paths.txt`

The local execution worker may write only inside `.notes/oracle/branch-trust-audit/lanes/04-window-flows/` and, if explicitly assigned by the manager, package-specific screenshots under `.artifacts/branch-trust-audit/04-window-flows/`. Production files are read-only. Shared article source, integrated ledger, manifest, root config, generated outputs, and publication are manager/lane-6 owned.
