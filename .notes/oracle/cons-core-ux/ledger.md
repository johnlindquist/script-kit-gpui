...
### C15 — UX-008 shared section presentation and stable leading slot

- **Status:** Complete; focused behavior/model tests, stable build, product runtime/visual matrix, negative controls, governance, anti-drift, and cleanup are green. C16 is next.
- **Implementation:** Added a pure family-aware section presentation resolver. Launcher display labels are uppercase and strong, with muted count/icon metadata; semantic labels retain authored bytes. Main `render_section_header`, Actions grouped headers, Unified section rendering, and Main semantic collection consume the shared model; Unified defaults to PreserveAuthored. Main result-cache normalization inserts an inert `ReservedSectionSlot` only when a non-empty launcher list lacks a real first header, before selection bounds and list-height calculation. Rendering, scrolling, navigation, sizing, preflight fingerprints, and Actions context mapping recognize that slot, while the semantic collector intentionally omits it.
- **Local implementation decision:** The existing tuple header variant remains the authored semantic row to avoid forcing broad source-data migration through hundreds of grouping fixtures. A distinct enum variant models the visual-only reserved slot more strongly than an optional empty label: it cannot carry semantic text or icon/count data, is compiler-exhaustive at every list owner, and is always inert. This satisfies the plan's no-empty-heading and stable-slot rationale without encoding absence as fake section content.
- **Focused receipts:** `render_section_header` 5/5; `section_header_presentation` 1/1; `actions::command_bar` 16/16; `actions::dialog` 5,807 passed/7 ignored/0 failed; `components::unified_list_item` 17/17; `agent-cargo check --lib` and product build finished. The brittle casing source-string assertion was replaced by resolver behavior coverage; no source reader was added.
- **Runtime/visual receipt:** `.artifacts/consistency/UX-008/runtime-section-grammar.json` is `RUNTIME-CONFIRMED`. Main's empty, `section`, `notes`, and `type:script` states all report first selectable Y `87`; a scoped zero-result query reaches zero and clearing restores Y `87`. Main semantic/display pairs retain authored `Suggested`/`Results` and render `SUGGESTED`/`RESULTS`. Actions exposes raw/display pairs and strong/muted tiers for four sections. Exact-generation Dictation compact rows expose no section. Three PNGs were visually inspected.
- **Negative controls:** Unicode uppercase expansion preserves semantic bytes; PreserveAuthored does not transform; metadata cannot inherit strong tier; reserved slot is inert and omitted at zero results; semantic collection cannot expose it as an empty heading; SectionStyle None/Separators behavior remains green; the runtime fails on a one-pixel first-row delta; compact no-header surface remains headerless.
- **Build:** `target-agent/artifacts/ux8-section-grammar/script-kit-gpui`; SHA-256 `9ce34b11ed6dc6e3a19204500d77ada296a13f8ed955d3730a3d0f04c79c68c1`.
