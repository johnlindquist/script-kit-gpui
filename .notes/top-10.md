...
Updated and re-audited: 2026-08-23

Baseline commit: `aee92338d`

Status: audited implementation roadmap, **not a shipping approval**. The
original baseline and current ownership-grouped committed source are different
states; clean Git history, passing source policies, and fixture receipts do
not replace current-source Rust compilation or real product-runtime proof.
...
- The strict synthetic SDK release suite executes **261 passing cases, zero
...
- The latest completed noninteractive release proof-contract checkpoint
  executed **1,402 passing tests across 47 files with 5,519 assertions**,
  with one bounded compiler/test worker. Later MCP discovery and diagnostic
  fixes have fresh direct 46-case behavior, 261-case shipping, SDK-type,
  and source-bound offline proof; repeating the broad release phase is
  intentionally deferred while workstation CPU load exceeds safe capacity.
  The release archive coverage
...
- A previous standalone non-GUI `export_design_tokens` run matched JSON/CSS
  bytes for its **older** source. That binary has no independently observed
  source-bound build manifest for the current checkout; the corrected verifier
  now refuses it before process startup instead of accepting today's commit
  label as fabricated build provenance. GOV-005 is therefore genuinely
  **INVALID / STALE** until a fresh, clean-source exporter build emits the
  wrapper's v2 artifact manifest and actual generated bytes pass again.
- The canonical current-source consistency auditor currently proves
  **13/75** tasks. The exact remaining breakdown is **60 missing
  direct-runtime obligations**, **GOV-003 blocked** because its fresh real
  compiler behavior cannot run below the 25 GiB disk floor, and **GOV-005
  invalid** because its old token-export receipt lacks current independent
  source/build provenance. The auditor correctly returns `INVALID_SCHEMA`;
  neither a stale artifact nor a historical 15/75 checkpoint is a current
  passing release proof. All **28** Flow safety/workflow journeys remain
  unproven. Protected-source, conflict-lifecycle, façade-lifecycle, privacy,
  and cleanup governance inventories pass only within their actual static or
...
- Focused synthetic security verification additionally closed four genuine
  ...
  subprocess, and the latest completed full release proof lane passed all
  **1,402 tests** before the independently verified MCP follow-up.
...
| 03. Design system | Partial | Shared presentation and canonical conversation/popup owners have freshly regenerated façade/conflict governance evidence, and every extracted design exporter owner is independently fingerprinted. Historical exporter JSON/CSS byte equality is not current proof: the available binary has no source-bound v2 build manifest and the corrected gate rejects it before execution. Fresh byte parity and surface-by-surface rendered parity remain unproven. |
...
| 08. SDK compatibility | Partial | The full isolated suite passes 261/0/0; separate filtered runs prove eight fields date/time/search cases, two hotkey cases, five editor/mini/micro cases, and 46 real socket-free local/remote/stdio MCP transport, private-discovery, complete inventory/failure credential projection, bounded streams, noninteractive child ownership, ordered low-concurrency discovery, hot configuration refresh, and orphan-cleanup cases; nine real Rust capability owners mark implemented prompts supported, mark all 18 genuinely unavailable capabilities unsupported, and enforce explicit native permission/platform boundaries. Generated scripts validate capabilities/topology and shared shell/slug policy, escape hostile JSON template values nonrecursively, and write complete final starter bytes through the original exclusive file handle; real seeded/template scripts declare parseable supported capabilities; Rust behavior plus the four repaired creation-contract cases prove collision/symlink-safe files and receipts. Native prompt/window/permission proof remains absent. |
| 09. Behavioral proof | Partial / fail-closed | The exact current-source auditor accepts 13/75: 60 direct-runtime tasks are missing, GOV-003 awaits real compiler behavior, and GOV-005 is explicitly invalid because the available exporter has no current independent build provenance. The two-façade lifecycle inventory and all 18 protected-source hashes are current; every extracted glass owner has individual omission/drift negatives; hidden identity rejects fabricated generations; all 28 Flow journeys remain unproven. The historical 1,402-case release phase is not current-source proof. |
| 10. Packaged release | Blocked honestly | The 65-violation production-size debt is resolved with no raised ceiling; bounded CPU/disk policy, strict library-plus-shipping-binary command ownership, SDK safety, protected calibration, source-bound release receipts, private storage, truthful AI failures, and signed-archive manifest semantics have fresh app-free negative controls. Current-source Rust compilation, exact strict Clippy, executed Rust tests, a rebuilt provenance-bound exporter, actual packaged journeys, the full direct matrix, signing/notarization, and ratified painted latency remain required and unverified. |
...
17. Protected cache names were only character-sanitized: a pool or exclusive
    agent ID of `.` / `..` resolved to its parent directory, and an artifact
    name of `..` escaped `target-agent/artifacts`. Existing symlinked pools,
...
49. The previous single-shot offline producer always executed the AlphaByte
    ...
    independently rejects every omitted owner and forged SDK matrix in
    **124 cases / 390
...
50. The real custom-script SDK verifier accepted an arbitrary absolute script
    ...
    all **261 actual authoring behaviors across 41 SDK script files** pass
...
51. The SDK verifier's final summary silently overwrote terminal outcomes in
    ...
    all **261/261 real SDK cases**, and its independent adversarial suite
...
53. Automatic SDK discovery still validated only a discovered symlink's
    ...
    remains **261/261 across 41 actual SDK files**, and **49 adversarial
...
54. The bounded Cargo owner still accepted `--manifest-path`, Cargo's `-C`
    ...
    179 assertions**, with no Cargo, Rust, Swift, application, or GUI launch.
55. The shipped `scriptkit` MCP CLI sent its private local bearer token to
    arbitrary remote/lookalike URLs, followed redirect policy implicitly,
    accepted world-readable or symlinked discovery files, returned
    `success: true` for genuine HTTP-200 JSON-RPC errors, never checked
    response identity, leaked credentials in three error formats, and could
    hang forever. Its existing tests required binding a local socket and
    failed outright inside the normal restricted environment. The actual
    embedded CLI now opens its user-owned **0600** discovery file with
    `O_NOFOLLOW`, permits only local `localhost` / `127.0.0.1` / `[::1]`
...
56. Compiler-source provenance still omitted **11 actual `include_str!`
    ...
    a credential-security or packaged-example fix could masquerade as
    equivalent current source. The canonical compiler-input owner now
    includes both real paths; source hashing, dirty-source rejection, APFS
    artifact export, staged build manifests, and cached harness reuse all
    consume the expanded closure. The fake-build owner test independently
    observes both additions through the exact Git source commands.
57. The embedded SDK's real `computer.*` and configured `mcp.*` clients had
    the same arbitrary-destination token exposure, response confusion,
    unlimited wait, unsafe diagnostics, and unowned stdio descendants.
    Worse, `skPath()`, `kitPath()`, private discovery, and config import
    ignored Rust's supported `SK_PATH`, so a custom workspace silently read
    another workspace's commands, credentials, and MCP servers. Both public
    paths, the SDK, and the CLI now resolve one active workspace; config
    cache entries are path-owned; self discovery is no-follow/owner-only and
    loopback-only; deliberately configured remote servers remain supported;
    all HTTP/stdio replies require their exact request identity; stderr is
    ...
    **46 real SDK MCP journeys** prove healthy local/remote/stdio behavior,
    ...
    isolated authoring suite now passes **261/261 behaviors across 41
...
58. The real shipping SDK gate previously trusted three unbound counters:
    ...
    whole-product release evidence even when **40 of 41 script families**,
    every sensitive MCP journey, and all actual test outcomes were omitted.
    The runner and verifier now share one tracked unsafe-system-input owner;
    release acceptance independently rediscovers every canonical reviewed
    `test-*.ts` file, excludes real keyboard/clipboard/scroll probes, rejects
    external/symlinked owners and duplicate/missing script receipts,
    validates each result identity/timestamp/duration/error/terminal state,
    ...
    **261 behaviors across all 41 script files**, including the exact
    **46-case** safe MCP transport owner; source, proof, and adversarial
...
61. The SDK cached workspace `config.ts` forever by path. Editing its
    ...
    **261/261 authoring behaviors across all 41 reviewed files**, including
    **46 isolated MCP journeys**, without a desktop or network action.
...
63. Documentation-only executable reuse was tested by asserting `HEAD^`
    ...
    passed **1,402 cases / 5,519 assertions across 47 files in 32.46s**
    with one worker; a full rerun after the subsequent directly proven MCP
    changes is deferred until CPU capacity returns. No application, browser,
    visible window, native input, capture, or live AI provider starts.
64. `mcp.listTools()` and AI-driven `mcp.discover()` used an unbounded
    ...
    matrix passes **261/261 behaviors across 41 scripts**, including
    **46 independently executed MCP journeys**, with one SDK worker.
65. Public MCP server metadata no longer displayed credentials, but actual
    failure messages still exposed remote endpoint usernames/passwords,
    URL query API keys, decoded or percent-encoded bearer values, cookie
    session secrets, custom secret headers, and configured stdio argv.
    Three real isolated HTTP/stdio error paths reproduced the cleartext
    disclosure despite successful safe inventory projection. HTTP and
    stdio diagnostics now independently derive their complete secret set
    from private environment, headers, cookie components, bearer/basic
    values, sensitive command-line arguments, endpoint credentials, and
    encoded query variants. Real authentication remains unchanged, while
    every observed failure is redacted before leaving the transport.
...
70. Splitting a calibrated owner silently invalidates any verifier that
    ...
    policy/design/coverage/protected-glass/observer suite passes **342 tests,
    zero failures, and 1,408 assertions across 10 files**; the named locked
...
71. The exact GOV-005 generated-byte verifier still accepted a previous
    executable without proving that executable was compiled from the source
    it claimed: checking today's `HEAD`, current Rust-owner hashes, matching
    binary bytes, and matching output bytes did **not** establish a
    relationship between that old binary and those current owners. The
    repository already had a stronger independently observed v2 build
    manifest and reviewed-compiler-tree verifier for real runtime proof;
    the exporter simply did not use it.

    Generated-byte production and receipt validation now share that same
    existing provenance authority. Before any exporter process or temporary
    directory starts, the exact owned artifact must have one non-symlinked
    manifest, matching binary path/hash/size, the complete current reviewed
    compiler-input tree, a clean build and current clean compiler sources,
    a valid build commit/profile/pool, and exact-commit enforcement wherever
    required. The resulting behavior receipt independently binds the
    manifest path/hash/schema/build source/current source/compiler tree and
    rejects changed provenance both during execution and later auditing.
    Missing manifests, dirty input, stale build commits, replaced binary
    bytes, forged identities, and swapped manifests all fail **before
    exporter execution** or invalidate the receipt afterward.

    The real available `export_design_tokens` artifact currently has **no
    `.provenance.json`**, and the actual command now fails closed with
    `verified build provenance requires exactly one independently observed
    artifact manifest`; the exporter is never launched. **37 focused
    generated-byte cases / 155 assertions** pass, including each real
    provenance-negative shape; the integrated provenance/runtime/workflow/
    consistency/Cargo suite passes **347 cases / 1,892 assertions**. The
    exact canonical auditor consequently reports **13/75 accepted**,
    **60 runtime tasks missing**, **GOV-003 blocked**, and **GOV-005
    invalid** until safe disk capacity allows a genuine current-source
    compiler/exporter build. This lower count is the corrected result of a
    stronger verifier, not permission to replay or relabel the old proof.
...
6. **Protect live and warm build caches.** A shared lock-owner contract makes
   ...
   escape into `target/` or an unrelated cache. Pool names, exclusive agent
   IDs, and artifact exports must identify one actual child rather than `.`
   or `..`; every cache root, selected pool, shared module cache, and export
   destination refuses preexisting symlinks before Cargo starts.
...
10. **Lock the contract into real behavior and release gates.** Dedicated
    ...
    **49 cases and 145 assertions**; the actual two-worker SDK runner
    separately passes **261 cases, zero failures, and zero skips**, including
    **46 isolated MCP local/remote/stdio and workspace-ownership journeys**.
...
not overwrite the current **261 passed / 0 failed / 0 skipped** worktree receipt
...
- The strict complete worktree suite currently reports **261 passed, 0 failed,
...
4. Preserve the current 261-case, zero-skip SDK fix and owner-registry repair;
   rerun stale
