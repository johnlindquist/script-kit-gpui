...
const tracePath = resolve(".test-output/quick-ai-fastest-search-trace.ndjson");
...
function traceSummary(records: Json[]) {
	const spawned = records.find((record) => record.event === "spawned") ?? {};
	const terminal = records.filter((record) => record.event === "terminal");
	const teardown = records.find((record) => record.event === "teardown") ?? {};
	const sources = records
		.filter((record) => record.event === "structured_source_observed")
		.map((record) => String(record.url));
	const finalAnswer =
		records.find((record) => record.event === "final_answer_selected") ?? {};
	const searchStarted = records.filter(
		(record) => record.event === "web_search_started",
	);
	const searchCompleted = records.filter(
		(record) => record.event === "web_search_completed",
	);
	...
	return {
		...
		searchStartedCount: searchStarted.length,
		searchCompletedCount: searchCompleted.length,
...
if (!existsSync(binary)) throw new Error(`Missing pinned artifact: ${binary}`);
...
try {
	...
	for (let trial = 1; trial <= 3; trial++) {
		...
		const officialStructuredSource = [...sourceHosts].some(isOfficialRustHost);
		const sourceProof =
			(officialStructuredSource && sourceAnswerHostIntersection) ||
			(summary.sourceProvenance === "answer_url_after_native_search" &&
		...
		const valid =
			...
			summary.searchStartedCount >= 1 &&
			summary.searchCompletedCount >= 1 &&
		...
		(receipt.trials as Json[]).push({
			...
			runId,
			dispatch,
			tabToSpawnMs: Number(spawned.elapsedMs ?? 0),
			...
			process: {
				...
				piProcessesSpawned: duringBackend.filter((row) =>
	...
	const knownRuns = new Set(parseTrace().map((record) => String(record.runId)));
	...
	while (performance.now() < cancelDeadline) {
		if (
			parseTrace().some(
				(record) =>
					record.runId === cancelRunId && record.event === "web_search_started",
			)
		) {
			break;
		}
	...
	receipt.cancellationTrial = {
		...
		cancellationMs: Math.round((performance.now() - cancelStarted) * 100) / 100,
	...
	const completeTimes = trials
		...
		.sort((a, b) => a - b);
	const medianCompleteTurnMs = completeTimes.length
		? completeTimes[Math.floor(completeTimes.length / 2)]
		: null;
	...
	receipt.summary = {
		...
		maxCompleteTurnMs: completeTimes.length ? Math.max(...completeTimes) : null,
		zeroContext: trials.every(
			(trial) =>
				trial.state.contextChipCount === 0 && trial.state.contextReady === true,
		),
		sourceProof: trials.every((trial) => trial.sourceProof),
		structuredSourceActions: trials.filter(
			(trial) => trial.search.sourceProvenance === "codex_web_search_action",
		).length,
		answerUrlAfterNativeSearchFallbacks: trials.filter(
			(trial) =>
				trial.search.sourceProvenance === "answer_url_after_native_search",
		).length,
