...
async function waitForWindowKind(
  ...
  while (performance.now() < deadline) {
    const receipt = responseOf(await driver.listAutomationWindows({ timeoutMs: 5_000 }));
...
async function waitForStateWhere(
  ...
  let last: Json = {};
  while (performance.now() < deadline) {
    last = responseOf(await driver.getState({ timeoutMs: Math.min(timeoutMs, 5_000) }));
...
async function main() {
  ...
  try {
    ...
    const fixtureReceipt = await driver.request(
      {
        ...
        commands: [{ type: "selectBySemanticId", semanticId: fixtureButton, submit: true }],
    ...
    const scriptList = (Array.isArray(layout.components) ? layout.components : [])
      .find((component: Json) => component.name === "ScriptList") as Json | undefined;
    const bounds = scriptList?.bounds as Json | undefined;
    if (!bounds || finite(bounds.x) === null || finite(bounds.y) === null
      || finite(bounds.width) === null || finite(bounds.height) === null) {
      throw new Error(`getLayoutInfo omitted ScriptList bounds: ${JSON.stringify(scriptList)}`);
    }
    const point = {
    ...
    const changedTopSamples = samples.filter((sample, index) =>
      index > 0 && Number(sample.scrollTop) !== Number(samples[index - 1].scrollTop));
    ...
    const rowSteppedScrollTopSequence = repeatedRowDeltas.length >= 2
      && repeatedRowDeltas.every((delta) => delta >= 32 && delta <= 64)
      && Math.max(...repeatedRowDeltas) - Math.min(...repeatedRowDeltas) <= 4;
    const wheelChangedSelection = Number(afterScroll.selectedIndex)
      !== Number(beforeScroll.selectedIndex);
    const wheelChangedViewport = Number(afterScroll.scrollTop)
      !== Number(beforeScroll.scrollTop);
    const focusedSemanticIdStable = typeof beforeScroll.focusedSemanticId === "string"
    ...
    const performancePopulated = performance?.enabled === true
    ...
    const missingSchemaFields = requiredScrollFields.filter((field) =>
      !Object.hasOwn(afterScroll, field));

    check("deterministic-fixture-opened", responseOf(fixtureReceipt).success === true, {
      semanticId: fixtureButton,
      batchSuccess: responseOf(fixtureReceipt).success ?? null,
    });
    check("main-list-scroll-schema-complete", missingSchemaFields.length === 0, {
      missingSchemaFields,
    });
    ...
    check("visible-range-populated", Number(afterScroll.lastVisibleIndexExclusive)
    ...
    check("frame-callback-proxy-metrics-populated", performancePopulated, performance ?? null);
    check("every-gpui-dispatch-succeeded", dispatchReceipts.every((receipt) =>
      receipt.success === true), { count: dispatchReceipts.length });
