...
| **Flow sessions (Threadline)** | Per-flow conversations with streaming turns, persisted transcripts (keyed by flow id + definition path so same-named flows in different projects never collide), reattach-on-restart, stale-definition re-thread, ⌘K Stop (turn only), ⌘L New Conversation (discards the transcript, keeps the session — refused with a neutral toast while a turn is in flight, and withheld from the ⌘K list entirely while working), ⇧⌘C Copy Last Response (newest NON-empty assistant turn — the in-flight turn is present with an empty body, and copying `""` succeeds silently), and ⇧⌘⎋ Terminate (ends the conversation AND erases its persisted history). Every chord the ⌘K list advertises is held against `resolve_flow_session_key_action` by `every_advertised_session_shortcut_has_a_declared_owner`; `⌘⇧D` Background is the one exception, answered by the window-level interceptor in `app_impl/startup.rs`. Answer text renders through the SHARED selectable conversation renderer, not Flow's own markdown engine. | `FlowSessionMeta`, `AppView::FlowSessionView` | [session.rs](file:///Users/johnlindquist/dev/script-kit-gpui/src/flows/session.rs) |
| **Shared conversation renderer** | The ONE owner of how an AI answer looks and behaves across Agent Chat and Flow: paint values (`conversation_style.rs`), the selectable `TextView` seam (`conversation_text.rs`), the per-turn copy control (`conversation_actions.rs`), and the jump-to-latest pill (`list_scroll_affordance.rs`). `agent_chat/ui/style_contract.rs` is a façade over it with zero production values. A turn's render identity is `ConversationTurnRenderKey`, resolved once from its ORIGINATING message — NOT `message_id`, which moves to the assistant's id on first reply. | `ConversationStyleDef`, `conversation_markdown_view`, `ConversationTurnRenderKey` | [conversation_style.rs](file:///Users/johnlindquist/dev/script-kit-gpui/src/components/conversation_style.rs) & [conversation_text.rs](file:///Users/johnlindquist/dev/script-kit-gpui/src/components/conversation_text.rs) |
