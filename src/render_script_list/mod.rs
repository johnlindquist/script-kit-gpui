...
struct MenuSyntaxMainHintFormContext<'a> {
    inputs: &'a [(String, Entity<gpui_component::input::InputState>)],
    field_bounds: std::rc::Rc<std::cell::RefCell<Vec<gpui::Bounds<gpui::Pixels>>>>,
}
...
fn render_menu_syntax_main_hint(
    ...
    list_tokens: crate::designs::MainMenuListTokens,
    design_variant: DesignVariant,
    form: MenuSyntaxMainHintFormContext<'_>,
    cx: &mut Context<ScriptListApp>,
) -> AnyElement {
    let MenuSyntaxMainHintFormContext {
        inputs: form_inputs,
        field_bounds: form_field_bounds,
    } = form;
    ...
    fn render_script_list(&mut self, cx: &mut Context<Self>) -> AnyElement {
            ...
            self.menu_syntax_main_hint_snapshot(&filter_text_for_render, false)
                .map(|hint| {
                    render_menu_syntax_main_hint(
                        ...
                        self.current_design,
                        MenuSyntaxMainHintFormContext {
                            inputs: &self.menu_syntax_form_inputs,
                            field_bounds: self.menu_syntax_form_field_bounds.clone(),
                        },
                        cx,
                    )
                })
        ...
        if is_mini {
            ...
            let mini_footer = if self.main_window_uses_native_footer() {
                crate::components::prompt_layout_shell::render_native_main_window_footer_hover_blocker()
                    .into_any_element()
            } else if let Some(config) = self.main_window_footer_config_with_cx(Some(&*cx)) {
                ...
                tracing::warn!("script_list.mini_footer_config_unavailable");
        ...
        let footer = {
            ...
            if let Some(config) = self.main_window_footer_config_with_cx(Some(&*cx)) {
                ...
                tracing::warn!("script_list.footer_config_unavailable");
                None
            }
        };
