...
impl ScriptListApp {
    ...
    fn dictation_history_actions_for_dialog(in_portal: bool) -> Vec<crate::actions::Action> {
        ...
        if !in_portal {
            actions.push(
                Action::new(
                    ...
                    Some(
                        "Hide Script Kit and paste this transcript into the active app".to_string(),
                    ),
        ...
        match subject {
            ...
            FlowDeskSubject::Session {
                ...
                for binding in flow_conversation_commands_for_facts(*facts) {
                    ...
                    let (action_id, description, section, icon): (&str, &str, &str, IconName) =
    ...
    pub(crate) fn execute_flow_desk_action(
        ...
        match (action_id, subject) {
            ...
            ("flow_desk_session_back_to_current", Some(FlowDeskSubject::Session { id, .. })) => {
                self.show_current_flow_conversation(id, cx);
            }
            ("flow_desk_session_history", Some(FlowDeskSubject::Session { id, archives, .. })) => {
                if let Some((archive_id, _)) = archives.last() {
                    self.show_flow_archive(id, archive_id, cx);
                }
            }
            (archive_action, Some(FlowDeskSubject::Session { id, .. }))
            ...
            ("flow_desk_session_continue_as_new", Some(FlowDeskSubject::Session { id, .. })) => {
                let archive_id = self
                    .conversations
                    .flow_sessions
                    .iter()
            ...
            ("flow_desk_session_new_conversation", Some(FlowDeskSubject::Session { id, .. })) => {
            ...
            ("flow_desk_recovery_primary", Some(FlowDeskSubject::Recovery { .. })) => {
