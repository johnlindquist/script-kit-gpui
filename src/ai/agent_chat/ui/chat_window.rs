        ...
        cx.spawn(async move |cx: &mut gpui::AsyncApp| {
            if let Ok(action_id) = action_rx.recv().await {
                ...
                cx.update(|cx| {
                    ...
                    // A parent-bound destructive confirmation owns focus until
                    // the user resolves it; reactivating its parent cancels it.
                    if handled
                        && !matches!(
                            action_id.as_str(),
                            "agent_chat_close" | "agent_chat_clear_history"
                        )
                    {
                        activate_chat_window(cx);
                    }
...
fn open_detached_history_actions(cx: &mut App) -> bool {
    ...
    cx.spawn(async move |cx: &mut gpui::AsyncApp| {
        if let Ok(action_id) = action_rx.recv().await {
            cx.update(|cx| {
                let handled = dispatch_detached_action_checked(&entity_weak, &action_id, cx);
                if handled && action_id != "agent_chat_clear_history" {
                    activate_chat_window(cx);
                }
                tracing::info!(
...
fn request_detached_agent_chat_history_deletion(
    owner: &WeakEntity<AgentChatView>,
    cx: &mut App,
) -> bool {
    if owner.upgrade().is_none() {
        tracing::warn!(event = "detached_history_deletion_owner_missing");
        return false;
    }
    ...
    let handle = {
        ...
        let Ok(guard) = slot.lock() else {
            tracing::warn!(event = "detached_history_deletion_window_unavailable");
            return false;
        };
        let Some(state) = guard.as_ref() else {
            tracing::warn!(event = "detached_history_deletion_window_unavailable");
        ...
        .update(cx, move |_root, window, cx| {
            crate::confirm::open_parent_confirm_dialog_for_entity(
                ...
                crate::confirm::ParentConfirmOptions::destructive(
                    "Delete All History?",
                    "Permanently delete every saved Agent Chat conversation and submitted prompt. This cannot be undone.",
                    "Delete All History",
                ),
                move |_window, cx| {
                    let Some(entity) = owner_for_confirm.upgrade() else {
                        tracing::warn!(event = "detached_history_deletion_owner_lost");
                        return;
                    };

                    let kit_path = crate::setup::get_kit_path();
                    match clear_agent_chat_history_at(
                        &kit_path,
                        Some(ConfirmedAgentChatHistoryDeletion),
                    ) {
                        Ok(()) => {
                            super::history::invalidate_history_cache();
                            entity.update(cx, |_chat, cx| cx.notify());
                            tracing::info!(event = "detached_history_deletion_confirmed");
                        }
                        Err(error) => {
                            tracing::warn!(
                                event = "detached_history_deletion_failed",
                                reason = error.safe_code(),
                                error_kind = ?error.io_error_kind(),
                            );
                        }
                    }
                },
                |_window, _cx| {
                    tracing::info!(event = "detached_history_deletion_cancelled");
                },
            );
        })
        .is_err()
    {
        tracing::warn!(event = "detached_history_deletion_window_update_failed");
        return false;
    }

    tracing::info!(event = "detached_history_deletion_confirmation_requested");
    true
}
...
fn dispatch_detached_action(
        ...
        crate::ai::agent_prompt_handoff::prompt_action_from_action_id(action_id)
    {
        if let Some(entity) = entity_weak.upgrade() {
            ...
            match result {
                Ok(receipt) => {
                    let safe_path = receipt
                        .path
                        .as_deref()
                        .map(crate::logging::log_private_user_value);
                    let safe_url = receipt
                        .url
                        .as_deref()
                        .map(crate::logging::log_private_user_value);
                    tracing::info!(
                        ...
                        url_bytes = ?safe_url.as_ref().map(|value| value.raw_bytes),
                        url_sha256 = ?safe_url.as_ref().map(|value| value.sha256.as_str()),
                    );
                }
                Err(error) => {
                    tracing::warn!(
    ...
    match action_id {
        ...
        "agent_chat_clear_history" => {
            let requested = request_detached_agent_chat_history_deletion(entity_weak, cx);
            tracing::info!(event = "detached_action_clear_history_requested", requested);
        }
