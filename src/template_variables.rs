...
fn add_builtin_variables(values: &mut HashMap<String, String>) {
    ...
    if !values.contains_key("datetime") {
        values.insert("datetime".to_string(), now.format("%Y-%m-%d %H:%M:%S").to_string());
    }
    ...
    if !values.contains_key("date_iso") {
        values.insert("date_iso".to_string(), now.format("%Y-%m-%dT%H:%M:%S%z").to_string());
    }
    ...
    match Clipboard::new() {
        Ok(mut clipboard) => {
            match clipboard.get_text() {
                Ok(text) => {
                    debug!(text_len = text.len(), "Retrieved clipboard text for variable substitution");
                    Some(text)
                }
                Err(e) => {
...
mod tests {
    ...
    fn test_date_variable_format() {
        ...
        assert!(result.contains('-'), "Date should contain dashes: {}", result);
    }
    ...
    fn test_time_variable_format() {
        ...
        assert!(result.contains(':'), "Time should contain colons: {}", result);
    }
    ...
    fn test_datetime_variable_format() {
        ...
        assert!(result.len() == 19, "Datetime should be 19 chars: {}", result);
        assert!(result.contains(' '), "Datetime should contain space: {}", result);
    }
    
    #[test]
    fn test_timestamp_is_numeric() {
        let result = substitute_variables("${timestamp}");
        assert!(result.parse::<i64>().is_ok(), "Timestamp should be numeric: {}", result);
    }
    ...
    fn test_year_is_four_digits() {
        ...
        assert!(result.parse::<u32>().is_ok(), "Year should be numeric: {}", result);
    }
    ...
    fn test_month_is_word() {
        ...
        assert!(months.contains(&result.as_str()), "Month should be full name: {}", result);
    }
    ...
    fn test_day_is_weekday() {
        ...
        assert!(days.contains(&result.as_str()), "Day should be weekday name: {}", result);
    }
    ...
    fn test_date_short_format() {
        ...
        assert!(result.len() == 10, "date_short should be 10 chars: {}", result);
        assert!(result.contains('/'), "date_short should contain slashes: {}", result);
    }
    ...
    fn test_time_12h_format() {
        ...
        assert!(result.contains("AM") || result.contains("PM"), 
                "time_12h should contain AM/PM: {}", result);
    }
    ...
    fn test_js_clipboard_syntax() {
        ...
        let result = substitute_variables_with_context(
